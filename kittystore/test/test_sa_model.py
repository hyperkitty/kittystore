...
from kittystore.sa.model import Email, Thread, User
...
class TestSAModel(unittest.TestCase):
    ...
    def tearDown(self):
        self.store.db.query(Thread).delete()
        self.store.db.query(Email).delete()
        self.store.close()
    ...
    def test_long_subject(self):
        ...
        msg.set_payload("Dummy message")
        self.store.add_to_list(ml, msg)
        msg_db = self.store.db.query(Email).one()
...
class TestStormModelVoting(unittest.TestCase):
    ...
    def tearDown(self):
        self.store.db.query(Thread).delete()
        self.store.db.query(Email).delete()
        self.store.db.query(User).delete()
        self.store.close()
